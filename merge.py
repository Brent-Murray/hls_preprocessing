...
def copy_median(merged_data, new_data, merged_mask, new_mask, **kwargs):
    """Returns the median value pixel."""
    mask = np.empty_like(merged_mask, dtype=bool)
    np.logical_or(merged_mask, new_mask, out=mask)
    np.logical_not(mask, out=mask)

    merged_data[mask] = np.median(np.stack((merged_data, new_data)), axis=0)[mask]
